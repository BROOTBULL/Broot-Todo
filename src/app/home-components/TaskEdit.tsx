...
export default function TaskEdit() {
  ...
  return (
    <div className="w-[30%] h-[100vh] fixed p-2 right-0 z-1">
        ...
        {selectedTodo ? (
          <>
            <div className="h-10 w-full border-b-2 rounded-md border-rose-800/60 px-2 flex items-center ">
              <div
                onDoubleClick={() => setEditlocation(true)}
                className={`text-base text-slate-300 hover:bg-slate-800 p-1 pl-2 pr-0 rounded-md cursor-pointer ${
              ...
              <div
                className={`${editLocation ? "flex" : "hidden"} flex-row gap-1`}
              >
                <button
                  onClick={() => {
                ...
                {sectionBox && (
                  <div className="bg-linear-to-tr from-slate-900 to-slate-800 h-fit max-h-50 w-50 absolute left-40 mt-8.5 rounded gap-1 flex flex-col p-2 z-2">
                    {projectData.length > 0 ? (
                      projectData
                        .find(
                          (project) => project._id === newLocation.projectId
                        )
            ...
            <div className="w-full rounded-md ml-auto flex my-5 ">
              <div
                onClick={() =>
                  setEditTask((prev) => ({ ...prev, status: prev.status==="pending"?"in-progress":"done"  }))
                }
                className={`hover:bg-emerald-900/50 ${editTask.status==="done"?"pointer-events-none":"bg-slate-800/80"} rounded-md p-1 duration-200 cursor-pointer flex flex-row items-center px-3 text-sm text-nowrap text-emerald-400/80`}
              >
                {editTask.status === "pending"
                  ? "Start Working"
                  :(editTask.status==="in-progress"?"Mark complete":"Done")}
              </div>
              <div className="flex flex-row w-full gap-1.5 justify-end">
                <DatePicker
                  ...
                  className="border-1 border-slate-600/50 p-2 rounded-md w-35 h-8 outline-0 cursor-pointer text-slate-300 text-sm tracking-wider"
                ...
                <div
                  ...
                  className={`h-8 px-3 text-sm border-1 text-slate-300 border-slate-600/50 ${
            ...
            <div className="rounded-md bg-slate-950/50 flex max-h-147 h-full w-full overflow-hidden border-1 border-slate-700">
              ...
              >
                <input
                  className="text-lg text-slate-300 outline-none"
                  type="text"
                  name="task"
                  autoComplete="off"
                  value={editTask.task}
                ...
                <TextareaAutosize
                  minRows={2}
                  className="text-sm text-slate-400 h-full px-1 resize-none scroll-auto outline-none custom-scroll"
                  name="description"
            ...
            <div className="mt-auto flex justify-end w-full">
              <button
                type="button"
                className="rounded-md p-1.5 px-4 text-base duration-200 hover:bg-slate-600 text-slate-200 bg-slate-700 m-1 cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                form="EditSubmit"
                className="rounded-md p-1.5 px-4 text-base duration-200 hover:bg-rose-950 text-slate-100 bg-rose-800/80 m-1 cursor-pointer"
              >
                Save Task
              </button>
            </div>
